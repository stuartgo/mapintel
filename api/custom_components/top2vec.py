...
class Top2Vec2(Top2Vec):
    def __init__(
        ...
        if document_ids is not None:
            ...
            self.doc_id2index = dict(
                zip(self.document_ids, list(range(0, len(self.document_ids))))
            )
            self.doc_id_type = np.int_
        ...
        if embedding_model == "doc2vec":
            ...
            tokenized_corpus = [
                tokenizer(doc) for doc in documents
            ]  # the tokenizer is important here because we have to build a vocabulary
            ...
            if len(vocab_inds) == 0:
                raise ValueError(
                    f"A min_count of {min_count} results in "
    ...
    def _check_import_status(self):
        if (
            self.embedding_model == "universal-sentence-encoder"
            or self.embedding_model == "universal-sentence-encoder-multilingual"
        ):
            if not _HAVE_TENSORFLOW:
                raise ImportError(
                    f"{self.embedding_model} is not available.\n\n"
