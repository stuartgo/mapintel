...
We also built a simple web application based on the MapIntel system shown in Figure \ref{app_ui}. It is composed by 4 main components: a \textbf{search box} at the top of the page that the user can use to write any natural language query, an \textbf{interactive scatter plot} that shows the UMAP projection of the document embeddings and allows the exploration of the corpus, a \textbf{sidebar} at the left where the user can specify any additional parameter or binary filter to the query and map, and a \textbf{list of results} at the bottom that contains the documents with the highest scoring for the specified query.
...
% Demonstrate the utility of the software for competitive intelligence and include prints of the built application.
...
% Say that the system can be applied in different contexts and not only on CI.
