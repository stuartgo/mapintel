...
all_files = [os.path.join(dp, f) for dp, dn, fn in os.walk(os.path.expanduser("./bbc")) for f in fn][1:]
corpus = []
...
doc2vec_model = models.doc2vec.Doc2Vec.load("./doc2vec_model")  # you can continue training with the loaded model!
...
for ax, X, top, tit in zip(axes.flatten(), [embedded_train_corpus, embedded_test_corpus], [train_topics, test_topics],
    ...
    # Set title
    ax.set_title(tit)
# plt.savefig("./tsne_news_embeddings.png")  # exports png to current directory
plt.show()
