...
def umap_plot(documents, unique_topics, query_label):
    ...
    for c, topic in zip(cat_to_color(unique_topics), unique_topics):
        ...
        hover_data = data[["title", "snippet"]].applymap(lambda x: "<br>".join(wrap(x, width=100)) if x else "")
        ...
        fig.add_trace(
            go.Scattergl(
                ...
                marker={"size": 4, "color": c, "opacity": 0.5, "symbol": "circle"},  # marker opacity
                name=topic,
    ...
    fig.add_trace(
        go.Scattergl(
            ...
            name=query_label,
            hovertemplate="<b>Query Marker</b><br><br>" + "<b>Query</b>: %{text}<br>" + "<extra></extra>",
        ),
    )
