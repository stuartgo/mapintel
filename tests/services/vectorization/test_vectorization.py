...
def test_model():
    ...
    if os.path.exists("./tests/services/vectorization/model/"):
        shutil.rmtree("./tests/services/vectorization/model/")
    if os.path.exists("./tests/services/vectorization/model_get/"):
        shutil.rmtree("./tests/services/vectorization/model_get/")
    if os.path.exists("./tests/services/vectorization/model.zip"):
        os.remove("./tests/services/vectorization/model.zip")
    if os.path.exists("./tests/services/vectorization/model_get.zip"):
        os.remove("./tests/services/vectorization/model_get.zip")
    # creates and saves mlflow model
    mlflow.sklearn.save_model(
        sk_model=vectorizer,
        path="./tests/services/vectorization/model/",
        serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE,
    )
    # loads model from saved files
    sklearn_pyfunc = mlflow.pyfunc.load_model(model_uri="./tests/services/vectorization/model/")
    # zips file
    shutil.make_archive("./tests/services/vectorization/model", 'zip', "./tests/services/vectorization/model/")
    with open("./tests/services/vectorization/model.zip", "rb") as f:
        response_post = client.post(
            "http://localhost:8000/vectorisation/model",
    ...
    with open("./tests/services/vectorization/model_get.zip", "wb") as f:
        f.write(response_get.content)
    shutil.unpack_archive("./tests/services/vectorization/model_get.zip", "./tests/services/vectorization/model_get/", "zip")
    sklearn_pyfunc_alt = mlflow.pyfunc.load_model(model_uri="./tests/services/vectorization/model_get/")
